...
d.Windows 搜索pagename使用findstr命令
e.Windows 路径使用"\\"
f.Windows .sh替换.bat
g.Windows _$RANDOM替换%random%
"""
...
def monkey_run():
    ...
    for file in glob.glob(os.path.join(script_path, '*.bat')):
        os.remove(file)
    ...
    if aw == 'yes':
        ...
        for i in range(n):
            ...
            promodel = os.popen(
                "adb -s " + dev + ' shell "cat /system/build.prop | grep ro.product.model="').readlines()  # 获取手机型号 windows 使用find代替grep
            # print "promodel：" + promodel
            # modelname = ('').join(promodel)  # 将list转为字符串
            modelname = promodel[0]  # 从list中取出第一个值
            model = modelname[17:].strip('\r\n')

            print "adb -s " + dev + ' shell "cat /system/build.prop | grep ro.product.brand="'

            proname = os.popen(
                "adb -s " + dev + ' shell "cat /system/build.prop | grep ro.product.brand="').readlines()  # 获取手机名称
            roname = proname[0]
            name = roname[17:].strip('\r\n')
            packagename = os.popen(
                "adb -s " + dev + ' shell "pm list packages | grep" ' + pakage_name).readlines()
            ...
            if pk == 'com.tianyancha.skyeye':
                ...
                if filedir:
                    ...
                    os.mkdir(script_path)
                devicedir = os.path.exists(script_path + name + '-' + model + '-' + dev)
                if devicedir:
                    # print "devicedir: " + devicedir
                    print "File Exist!\n"
                else:
                    os.mkdir(
                        script_path + name + '-' + model + '-' + dev)  # 按设备ID生成日志目录文件夹
                wl = open(script_path + name + '-' + model + '-' + dev + '-logcat' + '.bat',
                          'w')
                # wl.write('adb -s ' + dev + ' logcat -v time ACRA:E ANRManager:E System.err:W *:S')
                wl.write('adb -s ' + ds[i] + ' logcat -v time *:W')
                wl.write(
                    '> ' + script_path + '"' + name + '-' + model + '-' + dev + '"' + '/logcat_' + now + '%random%.txt\n')
                wl.close()

                if testmodel == '1':
                    wd = open(script_path + name + '-' + model + '-' + dev + '-device' + '.bat', 'w')
                    wd.write(
                        '\nadb -s ' + ds[
                            i] + ' shell monkey -p ' + pakage_name + ' -v -v --pct-syskeys 0 --monitor-native-crashes --throttle 50 -s $RANDOM -v -v -v ' + count)  # 选择设备执行monkey
                    wd.write(
                        ' time > ' + script_path + '"' + name + '-' + model + '-' + dev + '"' + '/monkey_log_' + now + '%random%.txt\n')
                    wd.write('echo 测试成功完成，请查看日志文件~\n')
                    wd.close()
                elif testmodel == '2':
                    wd = open(script_path + name + '-' + model + '-' + ds[i] + '-device' + '.bat', 'w')
                    wd.write(':loop')
                    wd.write('\nset /a num+=1')
                    wd.write('\nif "%num%"=="4" goto end')
                    wd.write(
                        '\nadb -s ' + dev + ' shell monkey -p ' + pakage_name + ' -v -v --pct-syskeys 0 --monitor-native-crashes --throttle 50 -s $RANDOM -v -v -v ' + count)  # 选择设备执行monkey
                    wd.write(
                        ' time > ' + script_path + '"' + name + '-' + model + '-' + dev + '"' + '/monkey_log_' + now + '%random%.txt\n')
                    wd.write('echo 测试成功完成，请查看日志文件~\n')
            ...
            for script_file in os.listdir(script_path):
                if os.path.isfile(os.path.join(script_path, script_file)) == True:
                    if script_file.find(dev + '-device.bat') > 0:
                    # if script_file.find('device.bat') > 0:
